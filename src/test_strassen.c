...
#define VERBOSE_PRINT_MATRIX 0x01
#define VERBOSE_PRINT_TIME 0x02
#define VERBOSE_PRINT_COMMENT 0x04

void mult_test(data_t *C, data_t *B, data_t *A, uint32_t dim, uint32_t v) {
  clock_t before, after;
  if(v&VERBOSE_PRINT_MATRIX) {
...
int main(int argc, char *argv[]) 
{
  ...
  }
  {
    ...
    mult_test(C, B, A, 1000, 6);
    free(A); free(B); free(C);
  }
  {
    uint32_t *A, *B, *C;
    uint32_t i;
    A = malloc(sizeof(uint32_t)*2049*2049);
    B = malloc(sizeof(uint32_t)*2049*2049);
    C = malloc(sizeof(uint32_t)*2049*2049);
    assert(A);
    assert(B);
    assert(C);
    for(i=0; i<2049*2049; i++) { A[i] = i; B[i] = 0;}
    for(i=0; i<2049; i++) {B[2050*i] = 1;}
    mult_test(C, B, A, 2049, 6);
    free(A); free(B); free(C);
  }
  {
    uint32_t *A, *B, *C;
